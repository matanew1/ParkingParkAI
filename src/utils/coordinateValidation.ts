...
export function validateCoordinateWithLogging(
  ...
  if (!isValidIsraeliCoordinate(lat, lon)) {
    if (isNaN(lat) || isNaN(lon)) {
      ...
      console.warn(
        `${context}: Latitude outside Israel bounds: lat=${lat} (expected ${ISRAEL_BOUNDS.MIN_LAT}-${ISRAEL_BOUNDS.MAX_LAT})`
      );
    } else if (lon < ISRAEL_BOUNDS.MIN_LON || lon > ISRAEL_BOUNDS.MAX_LON) {
      console.warn(
        `${context}: Longitude outside Israel bounds: lon=${lon} (expected ${ISRAEL_BOUNDS.MIN_LON}-${ISRAEL_BOUNDS.MAX_LON})`
      );
